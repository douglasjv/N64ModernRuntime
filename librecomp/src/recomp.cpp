...
#define LIBRECOMP_ANDROID_GAME_LOG(...) __android_log_print(ANDROID_LOG_INFO, "Librecomp", __VA_ARGS__)
#else
#define LIBRECOMP_ANDROID_GAME_LOG(...) ((void)0)
#endif
...
void recomp::start_game(const std::u8string& game_id, const std::string& game_mode_id) {
    ...
    if (game_status.load() == GameStatus::Running) {
        LIBRECOMP_ANDROID_GAME_LOG("start_game ignored while already running game=%s mode=%s",
            reinterpret_cast<const char*>(game_id.c_str()), game_mode_id.c_str());
        return;
    }
    LIBRECOMP_ANDROID_GAME_LOG("start_game requested game=%s mode=%s",
        reinterpret_cast<const char*>(game_id.c_str()), game_mode_id.c_str());
        ...
        case GameStatus::Running:
            {
                if (!recomp::load_stored_rom(current_game.value())) {
                    ...
                    LIBRECOMP_ANDROID_GAME_LOG("load_stored_rom ok size=%zu", recomp::get_rom().size());
                }

                auto find_it = game_roms.find(current_game.value());
                const recomp::GameEntry& game_entry = find_it->second;

                LIBRECOMP_ANDROID_GAME_LOG("game boot init entrypoint_addr=0x%08" PRIX32, game_entry.entrypoint_address);
                init(rdram, context, game_entry.entrypoint_address);
                if (game_entry.on_init_callback) {
                    LIBRECOMP_ANDROID_GAME_LOG("game boot on_init begin");
                    game_entry.on_init_callback(rdram, context);
                    LIBRECOMP_ANDROID_GAME_LOG("game boot on_init end");
                }
                ...
                LIBRECOMP_ANDROID_GAME_LOG("game boot init_heap mod_ram_used=%" PRIu32, mod_ram_used);
                recomp::init_heap(rdram, recomp::mod_rdram_start + mod_ram_used);

                save_type = game_entry.save_type;
                LIBRECOMP_ANDROID_GAME_LOG("game boot init_saving save_type=%d", int(save_type));
                ultramodern::init_saving(rdram);

                try {
                    LIBRECOMP_ANDROID_GAME_LOG("game boot entrypoint begin");
                    game_entry.entrypoint(rdram, context);
                    LIBRECOMP_ANDROID_GAME_LOG("game boot entrypoint returned");
                } catch (ultramodern::thread_terminated& terminated) {
                    LIBRECOMP_ANDROID_GAME_LOG("game boot entrypoint terminated");
                }
            }
            return true;
